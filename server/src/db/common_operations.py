...
def insertCustomer(db: SQLAlchemy, data: dict[str, str]) -> None:
    query = f'INSERT INTO Bank.Customers VALUES (:_id, :_fname, :_lname, :_dob)'
    db.session.execute(text(query), data)
    db.session.commit()

def insertAddress(db: SQLAlchemy, data: dict[str, str]) -> None:
    query = f'INSERT INTO Bank.Customer_Addresses VALUES (:_id, :_addr, :_addr_type)'
    db.session.execute(text(query), data)
    db.session.commit()

def insertPhone(db: SQLAlchemy, data: dict[str, str]) -> None:
    query = f'INSERT INTO Bank.Customer_PhoneNumbers VALUES (:_id, :_phone, :_phone_type)'
    db.session.execute(text(query), data)
    db.session.commit()

def insertUser(db: SQLAlchemy, data: dict[str, str]) -> None:
    query = f'INSERT INTO Bank.Users VALUES (:_username, :_password, :_id, :_user_type)'
    db.session.execute(text(query), data)
    db.session.commit()

def getUserByName(db: SQLAlchemy, username: str) -> Row[t.Any]:
    query = 'SELECT b.username AS user, b.user_type AS type, b.password ' \
